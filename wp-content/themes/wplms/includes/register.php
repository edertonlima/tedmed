...
class WPLMS_Register{
    ...
    function vibe_header_essentials(){
       ...
       if(!empty($favicon)){
          ...
          echo '<link rel="shortcut icon" href="'.$protocol.$favicon.'" />
              <link rel="icon" type="image/png" href="'.$protocol.$favicon.'">';
       }       
        ...
        if( ! is_admin() ){
         ...
         if(!empty($google_fonts) && is_array($google_fonts)){
            ...
            foreach($google_fonts as $font){
              ...
              if(!empty($font_weights[$font_var[0]]) && is_array($font_weights[$font_var[0]])){
         ...
         if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || (function_exists('is_plugin_active') && is_plugin_active( 'woocommerce/woocommerce.php'))) {     
          ...
          wp_enqueue_script( 'nprogress-js', VIBE_URL.'/assets/js/old_files/nprogress.js',array('jquery'),WPLMS_VERSION);
