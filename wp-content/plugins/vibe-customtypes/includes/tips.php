			...
			foreach($this->settings as $key=>$setting){
				switch($key){
					...
					case 'calculate_course_duration_from_start_course':
						...
						add_action('template_redirect',array($this,'extend_course'),10,1);
					break;
	...
	*/
	...
	function calculate_course_duration_from_start_course($course_id,$user_id){
		...
		if($time && time() > $time){
			...
			bp_course_get_user_expiry_time($user_id,$course_id);
	...
	function extend_course(){
		global $post,$wpdb;
		if($post->post_type == 'course' && is_user_logged_in() && function_exists('bp_is_active') && bp_is_active('activity')){
			$user_id = get_current_user_id();
			$course_id = $post->ID;
			$status = bp_course_get_user_course_status($user_id,$course_id);
			if($status == 1){ //USer is enrolled but has not started the course, hence extend his subscription
